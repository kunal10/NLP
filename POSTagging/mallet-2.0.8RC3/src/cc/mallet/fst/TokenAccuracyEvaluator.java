...
public class TokenAccuracyEvaluator extends TransducerEvaluator {
	private static Logger logger = MalletLogger
			.getLogger(TokenAccuracyEvaluator.class.getName());
	...
	public TokenAccuracyEvaluator(InstanceList[] instanceLists,
			String[] descriptions, HashSet<String> vocab) {
		super(instanceLists, descriptions);
		this.vocab = vocab;
	}
	...
	public TokenAccuracyEvaluator(InstanceList instanceList1,
			...
			String description2) {
		this(new InstanceList[] { instanceList1, instanceList2 },
				new String[] { description1, description2 });
	}

	public TokenAccuracyEvaluator(InstanceList instanceList1,
			String description1, InstanceList instanceList2,
			String description2, InstanceList instanceList3,
			String description3) {
	...
	public void evaluateInstanceList(TransducerTrainer trainer,
		...
		for (int i = 0; i < instances.size(); i++) {
			...
			// System.err.println ("TokenAccuracyEvaluator "+i+"
			// length="+input.size());
			...
			for (int j = 0; j < trueOutput.size(); j++) {
				totalTokens++;
				if (vocab != null
						&& !vocab.contains(input.get(j).toString())) {
					totalOovTokens++;
				}
			...
			// System.err.println ("TokenAccuracyEvaluator "+i+"
			// numCorrectTokens="+numCorrectTokens+" totalTokens="+totalTokens+"
			// accuracy="+((double)numCorrectTokens)/totalTokens);
		}
	...
	public double getAccuracy(String description) {
		...
		throw new IllegalArgumentException(
				"No accuracy available for instance list \"" + description
						+ "\"");
	}
	 ...
	 * @return
	 */
	public double getOovAccuracy(String description) {
		Double ret = oovAccuracy.get(description);
		if (ret != null)
			return ret.doubleValue();
		throw new IllegalArgumentException(
				"No oov accuracy available for instance list \"" + description
						+ "\"");
	}

	public static HashSet<String> computeVocab(InstanceList trainingData) {
