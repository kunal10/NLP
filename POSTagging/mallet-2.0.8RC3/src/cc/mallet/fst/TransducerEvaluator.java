...
public abstract class TransducerEvaluator {
	private static Logger logger = MalletLogger
			.getLogger(TransducerEvaluator.class.getName());

	// TODO consider storing the TransducerTrainer here also? Methods like
	// precondition() will be shorter and easier.
	...
	/**
	 * Evaluates a TransducerTrainer and its Transducer on the instance lists
	 * specified in the constructor. .
	 * <P>
	 * The default implementation calls the evaluator's
	 * <TT>evaluateInstanceList</TT> on each instance list.
	 *
	 * @param tt
	...
	public void evaluate(TransducerTrainer tt) {
		...
		for (int k = 0; k < instanceLists.length; k++)
			if (instanceLists[k] != null)
				evaluateInstanceList(tt, instanceLists[k],
						instanceListDescriptions[k]);
	}
	...
	protected void preamble(TransducerTrainer tt) {
		...
		if (tt instanceof TransducerTrainer.ByOptimization
				&& (opt = ((TransducerTrainer.ByOptimization) tt).getOptimizer()
						.getOptimizable()) instanceof Optimizable.ByValue)
			logger.info("Evaluator iteration=" + iteration + " cost="
					+ ((Optimizable.ByValue) opt).getValue());
		else
			logger.info("Evaluator iteration=" + iteration
					+ " cost=NA (not Optimizable.ByValue)");
	}

	/**
	 * If this returns false, then the body of the evaluate(TransducerTrainer)
	 * method will not run. Use this method to implement behaviors such as only
	 * evaluating every 5 iterations with <code>
